import { Button, Form, Input, message, Radio, Select, Upload, UploadFile, UploadProps} from "antd"
...
const StaffAdd = () => {
    ...
    const handleUpload = async (file: UploadFile) => {
        ...
        try {
            const response = await axiosClient.post(`${SETTINGS.URL_API}/v1/upload/single-handle`, formData, {
                headers: {
            ...
            if (axios.isAxiosError(error)) {
                ...
                if(statusCode === 400){
                    ...
                    messageApi.open({
                        type: "error",
                        content: "Chỉ dược upload hình .png, .gif, .jpg, webp, and .jpeg format allowed!",
                    });
                }
                return null
            } else {
                            ...
                            >
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-[15px]">
                                    <Form.Item
                                        ...
                                        rules={[
                                            ...
                                            { pattern: /^0\d{9}$/, message: "Số điện thoại chưa đúng" },
                                        ]}
                                    >
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-[15px]">
                                    <Form.Item
                                        ...
                                        name="password"
                                    >
                                        <Input type ="password" className="pl-3 block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray form-input" />
                                    </Form.Item>
                                    <Form.Item
                                        label={
                                            <span className="block mt-4 mb-3 text-sm text-gray-700 dark:text-gray-400">
                                ...
                                <div className="mt-2">
                                    <Form.Item name="active">
                                        <Radio.Group name="active">
                                            <Radio value={true} className="inline-flex items-center text-purple-600 form-radio focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray">
                                                <span className="ml-2 text-gray-600 dark:text-gray-400">Kích hoạt</span>
                                            </Radio>

                                            <Radio value={false} className="ml-6 inline-flex items-center text-purple-600 form-radio focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray">
                                                <span className="ml-2 text-gray-600 dark:text-gray-400">Tạm khóa</span>
                                            </Radio>
                                        </Radio.Group>
                                    </Form.Item>
                                </div>
