									...
									<tbody className="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
										{
											getAllStaffs?.data?.staffs_list.length > 0 ? (
												getAllStaffs?.data?.staffs_list.map((item: TStaff, i:number)=>{
													return (
														<tr key = {i}  className="text-gray-700 dark:text-gray-400">
															<td className="px-4 py-3">
																{
																	item.avatar && item.avatar !== null  ? (
																		<img className="w-[40px] h-[40px] rounded-full object-cover" src= {`${SETTINGS.URL_IMAGE}/${item.avatar}`} alt={item.fullname} />
																	) : (
																		<img className="w-[40px] h-[40px] rounded-full object-cover" src="/images/noavatar.png" alt={item.fullname} />
																	)
																}
															...
															<td className="px-4 py-3 text-sm">
																{
																	item.role === 1 ? ("Quản trị viên") : ("Thành viên")
																}
															</td>
															<td className="px-4 py-3 text-sm ">
																{
																	item.active ? (
																		<FaLock className="text-green-500 cursor-pointer m-auto" title="Đang kích hoạt" />
																	) : (
																		<FaLock className="text-red-500 cursor-pointer m-auto" title="Tài khoản bị khóa" />
																	)
																}
															</td>
															<td className="px-4 py-3">
																<div className="flex items-center space-x-4 text-sm">
																	...
																	className="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray" aria-label="Edit" >
																	...
																	</button>
																	{
																		user?._id !== item._id && (
																			<button 
																				...
																				className="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray" aria-label="Delete">
																					<svg className="w-5 h-5" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20" >
												...
												<tr className="text-gray-700 dark:text-gray-400">
													<td  colSpan={7} className="text-center py-3" >
														{
															keyword != null || email != null || phone != null ? ("Không tìm thấy") : ("Dữ liệu đang được cập nhật")
														}
													</td>
												</tr>
											)
